...
----------------------------------------------------------------------------------*/
// 처음에 실린더는 페이스 노말이 기본이니, 스무스 쉐이딩 하는거처럼 버텍스 노말을 기본 노말로 바꿔서 (메인에서서)
...
const cylinder = new Cylinder(gl, 32, { color: [0.9, 0.3, 0.1, 1.0] });
...
//-light direction이 추가되었는데, light의 위치를 나타냄. (light의 위치에서 원점을 보는 형태)
...
//-fragment shader에서 원래 vec3 lightDir = normalize(light.position - fragPos); 이렇게 써야 하는데, 그냥 lightdirection을
//-정의 하였으니 vec3 lightDir = normalize(light.direction); 로 씀. 대상이 원점이기도 해서.
//-vec3 reflectDir = reflect(-lightDir, norm);에서는 -lightDir을 쓰고 -을쓰는건, 빛 -> 대상 -> 반사 형태이니 빛 -> 대상에서
//-lightdir에 -를 취해줘야 방향이 맞음.
//나머진 뭐 동일함. 