export class Cone {
    ...
    constructor(gl, segments = 32, options = {}) {
        ...
        const topVertex = [0.0, halfH, 0.0]; // 원뿔 상단의 꼭짓점
        ...
        for (let i = 0; i < segments; i++) {
            ...
            // 각 face의 4개 정점 (CCW)
            positions.push(...topVertex);
            positions.push(x0_bot, -halfH, z0_bot);
            positions.push(x1_bot, -halfH, z1_bot);
            ...
            const A_x = x0_bot - topVertex[0];
            const A_y = -halfH  - topVertex[1];
            const A_z = z0_bot - topVertex[2];

            const B_x = x1_bot - topVertex[0];
            const B_y = -halfH  - topVertex[1];
            const B_z = z1_bot - topVertex[2];

                // cross(A,B)
            let nx = A_y * B_z - A_z * B_y;
            let ny = A_z * B_x - A_x * B_z;
            let nz = A_x * B_y - A_y * B_x;
            // normal이 안쪽을 향해서, - 부호 붙이기
            nx = -nx;
            ny = -ny;
            nz = -nz;
                // normalize
            let size = Math.hypot(nx, ny, nz);
            if (size === 0){
                size = 1;
            }
            nx = nx / size;
    ...
    computeVertexNormals() {
        ...
        for (let i = 0; i < vCount; i++) {
            ...
            //top vertex는 항상 vertex normal이 [0,1,0]
            if (i % 3 === 0){
            ...
            //현재 face normal
            const faceNormal1 = vec3.fromValues(
                this.faceNormals[i * 3    ],
                this.faceNormals[i * 3 + 1],
                this.faceNormals[i * 3 + 2]
            );

            let neighbor_vertex; //vertex index
            if (i % 3 === 1){
                neighbor_vertex = (i - 3 + vCount) % vCount; // 이전 face
            }
            else{
                neighbor_vertex = (i+3)%vCount; // 다음 face
            }
            const faceNormal2 = vec3.fromValues(
                this.faceNormals[neighbor_vertex * 3    ],
                this.faceNormals[neighbor_vertex * 3 + 1],
                this.faceNormals[neighbor_vertex * 3 + 2]
            );
            vec3.add(temp, faceNormal1, faceNormal2);

            
            vec3.normalize(temp, temp);
            //result_value는 normalize된 결과과

            this.vertexNormals[i * 3 ] = temp[0];
            this.vertexNormals[i * 3+1] = temp[1];
            this.vertexNormals[i *3+2] = temp[2];
        }
